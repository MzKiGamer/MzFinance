  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        <div className="airbnb-card p-5 flex flex-col min-h-[220px]">
          <h3 className="text-[22px] font-black mb-4 tracking-tighter">{t('thermometer')}</h3>
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-[20px] font-bold text-gray-400">{t('status')}</span>
              <span className="flex items-center gap-1 text-[20px] font-black uppercase" style={{ color: health.color }}>
                {health.label} <span className="text-[22px]">{health.emoji}</span>
              </span>
            </div>
            ...
            <p className="text-[18px] text-gray-400 font-bold uppercase tracking-tight">
        ...
        <div className="airbnb-card p-5 flex flex-col min-h-[220px]">
          <h3 className="text-[22px] font-black mb-4 tracking-tighter">{t('goals')}</h3>
          <div className="flex-1 space-y-4">
            {goals.slice(0, 3).map(goal => {
              ...
              return (
                <div key={goal.id} className="space-y-1.5">
                  <div className="flex justify-between items-center text-[19px] font-bold">
            ...
            {goals.length === 0 && <p className="text-[18px] italic text-gray-300 font-bold">{t('noGoals')}.</p>}
          </div>
        </div>

        <div className="airbnb-card p-5 flex flex-col min-h-[220px]">
          <h3 className="text-[22px] font-black mb-4 tracking-tighter">{t('cards')}</h3>
            ...
            {cards.slice(0, 4).map(card => {
              ...
              return (
                <div key={card.id} className="flex justify-between items-center py-1">
                  <span className="font-bold text-[20px] text-gray-500 truncate pr-2">{card.name}</span>
                  <span className="text-[20px] font-black text-red-600 whitespace-nowrap">{formatCurrency(spent)}</span>
                </div>
              );
            })}
            {cards.length === 0 && <p className="text-[18px] italic text-gray-300 font-bold">{t('noCards')}.</p>}
          </div>
        </div>

        <div className="airbnb-card p-5 flex flex-col min-h-[220px]">
          <h3 className="text-[22px] font-black mb-4 tracking-tighter">{t('patrimony')}</h3>
          {currentUser?.permissions.canViewPatrimony ? (
            <div className="space-y-4">
              <span className={`text-[24px] font-black tracking-tighter block ${getValueColor(stats.patrimonyTotal)}`}>{formatCurrency(stats.patrimonyTotal)}</span>
              <div className="grid grid-cols-1 gap-2.5">
                <div className="flex justify-between text-[18px] font-black uppercase text-gray-400 tracking-widest">
            ...
            <div className="flex-1 flex items-center justify-center text-gray-200 font-bold italic text-sm"><Lock size={18} className="mr-2"/> Blocked</div>
          )}
        </div>
      </div>
